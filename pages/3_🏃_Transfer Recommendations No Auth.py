...
def main():
    ...
    with col2:
        st.markdown("""
                 ...
                 ## _Transfer Recommendation - No Authentication_
    ...
    st.markdown("""#### If authentication is having problems do this:
 1. Open this page in another window and login: [https://fantasy.premierleague.com] 
 2. After a login, copy the team number presented at the URL (example, for https://fantasy.premierleague.com/entry/111222/event/16/, the team number is 111222)
 3. Enter the team number below and press enter:
""")
    ...
    if my_team_num is not "":
        st.markdown(" 4. Go to [https://fantasy.premierleague.com/api/my-team/{}/] and copy the raw data in the field below and press enter the text.".format(my_team_num))

        data = st.text_area("My Team Data", key=None, help="Raw Json from URL above [https://fantasy.premierleague.com/api/my-team/{}/]".format(my_team_num))
        ...
        if my_team is not None:
            for k in ['picks', 'chips']:
                my_team[k] = pd.DataFrame(my_team[k])
